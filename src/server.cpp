...
Others :
*************************************************/
int CServer::socket_bind(const char *ip, int nPort){
	int socketfd;
...
返回说明:0/-1
*************************************************/
int CServer::ev_select(int socketfd){
	int i = 0;
	int connfd;
...
/************************************************
Function : do_epoll
Description : IO多路复用epoll
Input : int socketfd
Output :
Return : 0/-1
Others :
*************************************************/
int CServer::do_epoll(int socketfd){
	int epollfd;
	int nfds;
...
void CServer::addfd(int epollfd,int fd,bool oneshot){
	epoll_event event;
	event.data.fd = fd;
...
void CServer::reset_oneshot(int epollfd,int fd){
	epoll_event event;
	event.data.fd = fd;
...
//ET模式的工作流程
void CServer::edge_trigger( epoll_event* events, int nfds, int epollfd, int socketfd ){
	char buf[ BUFMAXSIZE ];   
...
void CServer::add_event( int epollfd, int fd, bool enable_et ){
	epoll_event event;  
	event.data.fd = fd;   
